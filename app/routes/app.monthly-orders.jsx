...
export const loader = async ({ request }) => {
  ...
  if (customerId || location || companyLocationId) {
    try {
      ...
      // Get monthly order products summary
...
export default function MonthlyOrders() {
  const { filters, productSummary, error, totalOrders, totalProducts, totalValue } = useLoaderData();
  ...
  // Prepare data table rows for products
  ...
  return (
    <Page>
      <TitleBar title="Monthly Order Analysis" />
      <Layout>
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">
                Monthly Product Summary by Customer & Location
              </Text>
