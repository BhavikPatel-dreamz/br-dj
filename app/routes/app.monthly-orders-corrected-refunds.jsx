...
import { getMonthlyOrderProductsWithRefunds } from "../actions/fhr-orders-corrected-refunds.server.js";
...
export const loader = async ({ request }) => {
  ...
  if (customerId || location || companyLocationId) {
    try {
      ...
      // Get monthly order products summary with refunds (corrected version)
      ...
      error = "Failed to fetch monthly order data with refunds. Please try again.";
    }
  }
...
export default function MonthlyOrdersCorrectedRefunds() {
  const { 
    filters, 
    productSummary, 
    error, 
  ...
  const rows = (productSummary || []).map((product, index) => [
    ...
    product.refunded_quantity > 0 ? 
      `${product.refunded_quantity} refunded (${((product.refunded_quantity / product.gross_quantity) * 100).toFixed(1)}%)` : 
      "No refunds",
  ]);
  ...
  return (
    <Page>
      <TitleBar title="Monthly Orders - Corrected Refunds" />
      <Layout>
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">
                Monthly Product Summary (Net of Refunds) - CORRECTED VERSION
              </Text>
              <Text as="p" color="subdued">
                📊 Based on actual database schema: shopify.refund + shopify.order_line_refund
              </Text>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">
                Product Summary (Net of Refunds) - CORRECTED
              ...
              {isSearching && (
                <div style={{ textAlign: "center", padding: "20px" }}>
                  <Spinner size="small" />
                  <Text as="p">Analyzing monthly orders with refunds...</Text>
                </div>
              )}
              ...
              {refundedValue > 0 && (
                <Box padding="400" background="bg-surface-caution">
                  <Text as="p" variant="bodyMd" color="subdued">
                    💡 <strong>Refund Impact:</strong> This report shows accurate net quantities and values after subtracting refunds. 
