  ...
  try {
    ...
    const query = `
      ...
      SELECT 
        ...
        SUM(COALESCE(rp.refunded_quantity, 0)) as refunded_quantity,
        SUM(op.ordered_quantity - COALESCE(rp.refunded_quantity, 0)) as net_quantity,
        SUM(op.ordered_value) as gross_value,
        SUM(COALESCE(rp.refunded_value, 0)) as refunded_value,
        SUM(op.ordered_value - COALESCE(rp.refunded_value, 0)) as net_value,
        AVG(op.unit_price) as average_price,
        COUNT(DISTINCT op.order_id) as order_count,
        COUNT(DISTINCT CASE WHEN rp.line_item_id IS NOT NULL THEN op.order_id END) as orders_with_refunds,
        COUNT(*) as line_item_count
      FROM OrderedProducts op
      LEFT JOIN RefundedProducts rp ON op.line_item_id = rp.line_item_id
      GROUP BY 
        op.product_id,
        op.variant_id,
        op.product_name,
        ...
        op.vendor,
        op.product_type
      HAVING SUM(op.ordered_quantity - COALESCE(rp.refunded_quantity, 0)) > 0
    ...
    const summaryQuery = `
      ...
      RefundedProducts AS (
        SELECT 
          r.order_id,
          SUM(rl.quantity) as total_refunded_quantity,
          SUM(CAST(ol.price AS DECIMAL(10,2)) * rl.quantity) as total_refunded_value
        FROM brdjdb.shopify.refund AS r
        INNER JOIN brdjdb.shopify.refund_line_item AS rl ON r.id = rl.refund_id
        INNER JOIN brdjdb.shopify.order_line AS ol ON rl.line_item_id = ol.id
        INNER JOIN brdjdb.shopify.[order] AS o ON r.order_id = o.id
      ...
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN rp.order_id IS NOT NULL THEN op.order_id END) as orders_with_refunds,
        COUNT(DISTINCT op.product_id) as total_products,
        SUM(op.ordered_value) as gross_value,
        SUM(COALESCE(rp.total_refunded_value, 0)) as total_refunded_value,
        SUM(op.ordered_value - COALESCE(rp.total_refunded_value, 0)) as net_value
      FROM OrderedProducts op
      LEFT JOIN RefundedProducts rp ON op.order_id = rp.order_id
    `;
  ...
  try {
    ...
    const summaryQuery = `
      ...
      SELECT 
        COUNT(DISTINCT op.order_id) as total_orders,
        COUNT(DISTINCT CASE WHEN rp.order_id IS NOT NULL THEN op.order_id END) as orders_with_refunds,
        COUNT(DISTINCT op.category_name) as total_categories,
        SUM(op.ordered_value) as gross_value,
        SUM(COALESCE(rp.total_refunded_value, 0)) as total_refunded_value,
        SUM(op.ordered_value - COALESCE(rp.total_refunded_value, 0)) as net_value
      FROM OrderedProducts op
